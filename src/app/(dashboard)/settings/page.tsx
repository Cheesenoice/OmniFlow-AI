...
export default function SettingsPage() {
  ...
  const addRssSource = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setRssMsg(`Failed: ${data.error}`);
      }
    } catch {
      setRssMsg("Network error.");
    } finally {
      setRssLoading(false);
    }
  };
  ...
  const saveSchedule = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setScheduleMsg(`Failed: ${data.error}`);
      }
    } catch {
    ...
    (async () => {
      try {
        const res = await fetch(
          "/api/settings/social-credentials?platform=facebook",
        );
        const data = await res.json();
        if (data.credentials?.access_token)
          setFbToken(data.credentials.access_token);
  ...
  useEffect(() => {
    ...
    if (t) {
      try {
        ...
        setTgToken(p.botToken ?? "");
        setTgChatId(p.chatId ?? "");
        setTgSaved(true);
      } catch {
        /* */
      }
    ...
    if (f) {
      try {
        ...
        setFbToken(p.pageAccessToken ?? "");
        setFbPageId(p.pageId ?? "");
        setFbSaved(true);
      } catch {
        /* */
      }
  ...
  const saveTelegram = () => {
    ...
    localStorage.setItem(
      "omni_telegram",
      JSON.stringify({ botToken: tgToken, chatId: tgChatId }),
    );
    setTgSaved(true);
    setTgResult(null);
  };
  ...
  const testTelegram = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setTgResult(
        data.success ? "OK! Check Telegram." : `Failed: ${data.error}`,
      );
    } catch {
  ...
  const saveFacebook = async () => {
    ...
    localStorage.setItem(
      "omni_facebook",
      JSON.stringify({ pageAccessToken: fbToken, pageId: fbPageId }),
    );
  ...
  const testFacebook = async () => {
    ...
    try {
      const res = await fetch("/api/publish", {
        ...
        body: JSON.stringify({
          platform: "facebook",
          content: "Test post from OmniFlow AI — connection works!",
          title: "OmniFlow AI Test",
      ...
      setFbResult(
        data.success
          ? `OK! Post ID: ${data.platformPostId}`
          : `Failed: ${data.error}`,
      );
    } catch {
      setFbResult("Network error.");
    } finally {
      setFbTesting(false);
    }
  };
  ...
  return (
    <div className="grid gap-6">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground flex items-center gap-1">
              <Check className="size-3 text-green-600" /> Set via GEMINI_API_KEY
              in .env.local
            </p>
          </div>
          <Separator />
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Cpu className="size-5 text-muted-foreground" /> AI Provider
          </CardTitle>
          <CardDescription>
        ...
        <CardContent className="space-y-3">
          ...
          {aiProvider === "groq" && (
            <p className="text-xs text-green-600 flex items-center gap-1">
              <Check className="size-3" /> Groq free tier: ~500-800 tok/s, cực
              nhanh.
            </p>
          )}
          {aiProvider === "gemini" && (
            <p className="text-xs text-muted-foreground">
              Gemini free tier: 1500 req/ngày với flash.
            </p>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Globe className="size-5 text-blue-600" /> Facebook Page
          </CardTitle>
          <CardDescription>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              disabled={!fbSaved || fbTesting}
            >
              <Globe className="size-4 mr-1.5" />{" "}
              {fbTesting ? "Testing..." : "Test Post"}
            </Button>
          </div>
          {fbResult && (
            <p
              className={`text-sm p-3 rounded-lg ${fbResult.startsWith("OK") ? "bg-green-50 text-green-700 dark:bg-green-950/20" : "bg-red-50 text-red-700 dark:bg-red-950/20"}`}
            >
              {fbResult}
            </p>
          )}
          {fbSaved && (
            <p className="text-xs text-green-600 flex items-center gap-1">
              <Check className="size-3" /> Facebook credentials saved
              (localStorage).
            </p>
          )}
          <Separator />
          <div className="text-xs text-muted-foreground space-y-1">
            ...
            <ol className="list-decimal ml-4 space-y-0.5">
              ...
              <li>
                Select your Meta App (or create one at developers.facebook.com)
              </li>
              <li>
                Click <strong>Get User Token</strong> → check{" "}
                <code className="bg-muted px-1 rounded text-xs">
                  pages_manage_posts
                </code>
              </li>
              <li>
                After getting User Token, click the{" "}
                <strong>Get Page Access Token</strong> button (blue (i) icon)
              </li>
              <li>
                Select your Page → copy the <strong>Page Access Token</strong>
              </li>
              <li>
                To get <strong>Page ID</strong>: in the same tool, send GET{" "}
                <code className="bg-muted px-1 rounded text-xs">
                  /me/accounts
                </code>{" "}
                → find your page
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Send className="size-5 text-blue-600" /> Telegram Bot (Quick Test)
          </CardTitle>
          <CardDescription>
            Fast alternative for testing publish flow.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
          ...
          {tgResult && (
            <p
              className={`text-sm p-3 rounded-lg ${tgResult.startsWith("OK") ? "bg-green-50 text-green-700 dark:bg-green-950/20" : "bg-red-50 text-red-700 dark:bg-red-950/20"}`}
            >
              {tgResult}
            </p>
          )}
          {tgSaved && (
            <p className="text-xs text-green-600 flex items-center gap-1">
              <Check className="size-3" /> Saved (localStorage).
            </p>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Rss className="size-5 text-orange-600" /> RSS News Sources
          </CardTitle>
          <CardDescription>
            Add RSS/Atom feeds to crawl. Articles are stored and can be used as
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
            ...
            <div className="w-20 space-y-1">
              ...
              <Input
                ...
                max={50}
                value={newMaxPosts}
                onChange={(e) => setNewMaxPosts(Number(e.target.value) || 10)}
              />
            </div>
            <Button
              onClick={addRssSource}
              disabled={!newName || !newFeedUrl || rssLoading}
              className="gap-1.5"
            >
          ...
          {rssMsg && (
            <div
              className={`text-sm p-3 rounded-lg ${rssMsg.startsWith("Source") ? "bg-green-50 text-green-700 dark:bg-green-950/20" : "bg-red-50 text-red-700 dark:bg-red-950/20"}`}
            >
              {rssMsg}
            </div>
          )}
          ...
          {rssSources.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">
              No RSS sources configured. Add one above.
            </p>
          ) : (
            <div className="space-y-2">
              {rssSources.map((src) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                      ...
                      {src.last_crawled_at && (
                        <span>
                          Last crawl:{" "}
                          {new Date(src.last_crawled_at).toLocaleDateString()}
                        </span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Button
                      ...
                      onClick={() => deleteRssSource(src.id)}
                    >
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Timer className="size-5 text-muted-foreground" /> News Automation
          </CardTitle>
          <CardDescription>
            Configure automated crawl schedule and auto-publish new articles to
            social media.
          </CardDescription>
        </CardHeader>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Auto-crawl RSS feeds on a schedule. Requires a cron job
                  calling /api/crawl/check.
                </p>
              </div>
              <Button
              ...
              <div className="space-y-3 pl-1">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    {(["interval", "daily", "weekly", "monthly"] as const).map(
                      (mode) => (
                        <Button
                          key={mode}
                          variant={
                          ...
                          className="h-7 text-xs"
                          onClick={() =>
                            setSchedule({ ...schedule, schedule_mode: mode })
                          }
                        >
                          {mode === "interval"
                            ? "Interval"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setSchedule({
                          ...schedule,
                          interval_minutes: Number(e.target.value) || 10,
                        })
                      }
                    />
                    ...
                    <span className="text-xs text-muted-foreground ml-2">
                      ...
                      {schedule.last_triggered_at
                        ? new Date(
                            new Date(schedule.last_triggered_at).getTime() +
                              schedule.interval_minutes * 60000,
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setSchedule({
                          ...schedule,
                          monthly_time: e.target.value + ":00",
                        })
                      }
                    />
                  </div>
                ...
                <div className="flex items-center gap-2 pt-1">
                  ...
                  {scheduleMsg && (
                    <span
                      className={`text-xs ${scheduleMsg.startsWith("Schedule") ? "text-green-600" : "text-red-600"}`}
                    >
                      {scheduleMsg}
                    </span>
                  )}
                  {schedule.last_triggered_at && (
                    <span className="text-xs text-muted-foreground ml-auto">
                      Last crawl:{" "}
                      {new Date(schedule.last_triggered_at).toLocaleTimeString(
                        "vi-VN",
          ...
          <div className="space-y-4">
            <div>
              <h3 className="font-medium text-sm flex items-center gap-1.5">
                <Rocket className="size-4 text-muted-foreground" /> Auto Publish
              </h3>
              <p className="text-xs text-muted-foreground mt-0.5">
                When new articles are crawled, automatically generate a social
                post and publish to the configured platform.
              </p>
            </div>

            {rssSources.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-4">
                Add RSS sources above to configure auto-publish.
              </p>
            ) : (
              <div className="space-y-3">
                {rssSources.map((src) => (
                  <div
                    key={src.id}
                    className="flex items-center justify-between gap-3 py-2.5 px-3 rounded-lg border bg-muted/30"
                  >
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium truncate">{src.name}</p>
            ...
            <p className="text-xs text-muted-foreground flex items-center gap-1.5">
              ...
              Requires Facebook credentials above and the cron endpoint to be
              active.
            </p>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Mail className="size-5 text-muted-foreground" /> Email
            Notifications
          </CardTitle>
          <CardDescription>
            Get notified when articles are auto-published. Uses Gmail SMTP with
            App Password.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              onClick={() =>
                setEmailCfg({
                  ...emailCfg,
                  notify_on_publish: !emailCfg.notify_on_publish,
                })
              }
            >
          ...
          {emailCfg.notify_on_publish && (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEmailCfg({ ...emailCfg, smtp_host: e.target.value })
                    }
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEmailCfg({ ...emailCfg, smtp_user: e.target.value })
                    }
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEmailCfg({ ...emailCfg, smtp_pass: e.target.value })
                    }
              ...
              <div className="flex items-center gap-2">
                ...
                {emailMsg && (
                  <span
                    className={`text-xs ${emailMsg.startsWith("Email") ? "text-green-600" : "text-red-600"}`}
                  >
                    {emailMsg}
                  </span>
                )}
              ...
              <p className="text-xs text-muted-foreground flex items-center gap-1.5">
                <AlertCircle className="size-3" />
                Tạo App Password tại Google Account → Security → 2-Step
